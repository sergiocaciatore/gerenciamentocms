...
export default function CollaboratorForm() {
  ...
  const [formData, setFormData] = useState<Collaborator>(initialCollaborator);
  ...
  useEffect(() => {
    const fetchCollaborators = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "AMZ_Collaborators"));
  ...
  const selectCollaborator = (collaborator: Collaborator) => {
    ...
    setSearchTerm(collaborator.name);
  ...
  const clearForm = () => {
    setFormData(initialCollaborator);
    setSearchTerm("");
  };
  ...
  const handleSave = async () => {
    // Basic validation
    if (!formData.name) {
      showModal("Campo Obrigatório", "Por favor, preencha o Nome Completo do colaborador.", "warning", undefined, true);
      return;
    }
    
    showModal(
        "Confirmar Cadastro",
        `Deseja ${formData.id ? 'atualizar' : 'cadastrar'} o colaborador "${formData.name}"?`,
        "info",
        async () => {
            setLoading(true);
            try {
                if (formData.id) {
                    ...
                    await updateDoc(doc(db, "AMZ_Collaborators", id), dataToSave);
                    
                    setCollaborators(prev => prev.map(c => c.id === id ? formData : c));
                    showModal("Sucesso", "Colaborador atualizado com sucesso!", "success", () => clearForm(), true);
                } else {
                    // Create new
                    const { id, ...dataToSave } = formData;
                    void id; // Silence unused variable lint
                    
                    const newId = await generateSequentialId("AMZ-COL");
                    await setDoc(doc(db, "AMZ_Collaborators", newId), dataToSave);
                    
                    const newCollaborator = { ...formData, id: newId };
                    setCollaborators((prev) => [...prev, newCollaborator]);
                    showModal("Sucesso", "Colaborador cadastrado com sucesso!", "success", () => clearForm(), true);
                }
            } catch (error) {
                console.error("Erro ao salvar colaborador:", error);
                showModal("Erro", "Erro ao salvar colaborador. Tente novamente.", "danger", undefined, true);
            } finally {
                setLoading(false);
            }
        }
  ...
  const handleDelete = () => {
      ...
      showModal(
          "Excluir Colaborador",
          `Tem certeza que deseja excluir o colaborador "${formData.name}"? Esta ação não pode ser desfeita.`,
          "danger",
          async () => {
              setLoading(true);
              try {
                  await deleteDoc(doc(db, "AMZ_Collaborators", formData.id as string));
                  setCollaborators(prev => prev.filter(c => c.id !== formData.id));
                  showModal("Excluído", "Colaborador excluído com sucesso.", "success", () => clearForm(), true);
              } catch (error) {
                  console.error("Erro ao excluir colaborador:", error);
                  showModal("Erro", "Erro ao excluir colaborador. Tente novamente.", "danger", undefined, true);
              } finally {
                  setLoading(false);
              }
          }
    ...
    <div className="flex flex-col h-full relative">
      <div className="p-4 md:p-8 space-y-8">
        ...
        <div className="relative z-10">
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Buscar Colaborador (Editar Existente)
          </label>
                  ...
                  filteredCollaborators.map((collaborator, index) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-500">
                        CPF: {collaborator.cpf} - {collaborator.role}
                      </div>
                    </div>
                  ))
                ) : (
      ...
      <div className="mt-8 flex justify-end gap-3 p-4 md:p-8 pt-0">
        ...
        >
          ...
          {loading ? 'Salvando...' : (formData.id ? 'Atualizar Colaborador' : 'Cadastrar Colaborador')}
        </button>
      </div>
