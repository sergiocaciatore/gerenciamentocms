                                ...
                                <div key={planning.id} className={`rounded-2xl bg-white/60 backdrop-blur-xl border border-white/60 shadow-lg overflow-hidden ${expandedId === planning.id ? 'ring-2 ring-blue-500/20' : ''}`}>
                                    <div className="p-6 cursor-pointer flex items-center justify-between" onClick={() => toggleExpand(planning.id, planning)}>
                                        ...
                                        <div className="flex items-center gap-1">
                                            <button className="p-2 text-gray-400 hover:text-blue-600" onClick={(e) => { e.stopPropagation(); setPlanningToEdit(planning); setEditForm({ status: planning.status || 'Rascunho' }); setIsEditModalOpen(true); }}>Edit</button>
                                            <button className="p-2 text-gray-400 hover:text-red-500" onClick={(e) => { e.stopPropagation(); setPlanningToDelete(planning.id); setIsDeleteModalOpen(true); }}>Del</button>
                                        </div>
                                    </div>
